...
# Read in hyperparameters from parameters file for all Caret ridge models
...
for (sampsize in samplesize_grid) {
  for (dataset_i in 1:length(datasets_grid)) {
    ...
    for (estimator_i in 1:length(estimator_grid)) {
      ...
      estimate_i <-
        tryCatch( {
          #If ridge RF, use hyperparameters selected from tuning
          
          training_time_start <- Sys.time()
          if (substr(estimator_name, 1, 5) == "caret") {
            Er <- forestry(x = as.data.frame(Xtrain),
                           y = Ytrain,
                           ridgeRF = TRUE,
                           nodesizeSpl = 1,
                           nodesizeAvg = 1,
                           nodesizeStrictAvg = 1,
                           nodesizeStrictSpl = params$nodesizeStrictSpl[1],
                           mtry = params$mtry[1],
                           overfitPenalty = params$overfitPenalty[1],
                           saveable = FALSE)
